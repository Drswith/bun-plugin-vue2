...
test('resolve function - absolute path', async () => {
  ...
  try {
    // 直接测试resolve工具函数
    const { tryResolveRealFileOrType, createResolvedId, DEFAULT_EXTENSIONS } = await import('../src/utils/resolve')
    
    // 测试绝对路径解析
    const resolved = tryResolveRealFileOrType(testFile, DEFAULT_EXTENSIONS, false)
...
test('resolve function - relative path resolution', async () => {
  ...
  try {
    const { tryResolveRealFileOrType, DEFAULT_EXTENSIONS } = await import('../src/utils/resolve')
...
test('resolve function - package resolution', async () => {
  const { findNearestPackageData, isDirectory } = await import('../src/utils/resolve')
...
test('resolve function - bare import patterns', async () => {
  ...
  expect(deepImportRE.exec('@vue/compiler-sfc/dist/index.js')?.[2]).toBe('@vue/compiler-sfc')
})
